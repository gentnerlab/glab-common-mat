function [Units] = SM_reads2mat_20110328_st421(infile,readSpikeShapes,verbose,iscompressedfile)
...
if nargin < 4,  iscompressedfile = 1;   end
...
subfilestartTMinds = find(TM.codes(:,1)==255);                      %find all subfile date/timestamp TMs
...
for subfilenum  = 1:length(subfilestartTMinds)                                          %get some basic data from each subfile textmark
   ttext = TM.text(subfilestartTMinds(subfilenum),:);
    dind = strfind(ttext,'-');
    subfiledate{subfilenum} = strrep(ttext(1:dind-1),':','-');
    
    HoursMinsSecs = textscan(ttext(dind+1:end),'%2d %*[:] %2d %*[:] %2d');
...
%     if length(subfileTD)==1 %takes care of old style timestamp format
...
trialIDTMinds = find(TM.codes(:,1)~=244 & TM.codes(:,1)~=243 & TM.codes(:,1)~=242 & TM.codes(:,1)~=255);                            %find all textmark ID TMs

%find trial starts
if iscompressedfile == 1
trialstartinds = find(DM.codes(:,1) == 42); %Currently, using '*' (ascii = 42, given in compress concat code) as trial delimeter - Need to make back compatible)
end
%find trial ends
...
if iscompressedfile == 1
   if length(trialendinds) ~= length(trialstartinds)
      disp('we have an issue #2: number of trial starts does not equal number of trial ends. I dont know what to do, build better error fixing!');
    return 
   end
end
...
doUSETMTIMESTAMPS = 0;
if doUSETMTIMESTAMPS == 1
    aa = TM.text(trialTimeStampTMinds,:);
    trialstarttimes = zeros(size(aa,1),1);
    for i  = 1:size(aa,1)
       trialstarttimesReltoSubFile(i) = str2double(aa(i,:));
    end
end
...
for trialnum = 1:size(stimstartinds,1)
    ...
    if doUSETMTIMESTAMPS == 1 %uses the time since sampling start stored in the TMCode244 text to calculate trial time
    trials{trialnum,3}      = subfilesecondssincemidnight(currsubfile) + trialstarttimesReltoSubFile(trialnum);   %   SECONDS SINCE MIDNIGHT for trial
    trials{trialnum,4}      = trialstarttimesReltoSubFile(trialnum);                                              %   time relative to subfile start
    else
    ...
    timeepsilon = 0.0001; %The smallest represented time difference in timing in seconds (should be 0.0001 seconds)                          %	timeepsilon avoids assigning two TM to same trial, as well as double counting spikes - this ensures each spike belongs to one trial
   if iscompressedfile == 1
       startOffset = (DM.times(trialstartinds(trialnum))-DM.times(stimstartinds(trialnum)))+timeepsilon;                                    %	timeepsilon avoids assigning two TM to same trial, as well as double counting spikes - this ensures each spike belongs to one trial
    else
    if trialnum == 1
        startOffset = -2;                                                                                                                   %	-2 given by trigger settings in the .smr file during recording itself
    else
    startOffset             = max(-2,(DM.times(trialendinds(trialnum-1))-DM.times(stimstartinds(trialnum)))+timeepsilon);                   %	timeepsilon avoids assigning two TM to same trial, as well as double counting spikes - this ensures each spike belongs to one trial
    end
end
    ...
    %save textmark data
    currTMind = trialIDTMinds(DM.times(stimstartinds(trialnum))+startOffset <= TM.times(trialIDTMinds) & TM.times(trialIDTMinds) <= DM.times(stimstartinds(trialnum))+endOffset); %look within trial range for textmarkID
...
subjectid   = regexp(strrep(infile,'\','/'),'st\d+','match','once');
