function Units = SM_readlib(subject,keepspikeshapes,dataonFugl2,verbose,noProcess,noGetSortQual)
...
if dataonFugl2 == 0
    ...
    libraryfile = fixfilesep(fullfile(subjectDir,strtrim(ls([subjectDir 'st*s2MATLibraryZ.txt']))));
else
...
fid = fopen(fixfilesep([subjectDir '/catfiles/' libraryfile]));
...
for currS2Mat=1:nums2mats
    ...
    Units = [Units SM_reads2mat(currS2MatName,keepspikeshapes,verbose)];
end
                ...
                for sortfileNum = 1:size(Sort.sortfiles,1)
                    if ~isempty(findstr(Sort.sortfiles{sortfileNum}(1:end-4), Units(unitNum).info(sortNum).s2MATfile))
                        if Sort.markers(sortfileNum) == Units(unitNum).marker %after here should have correct index - but keep looking for completeness
                            if strcmp(Sort.sites{sortfileNum},Units(unitNum).site)
                                if strcmp(Sort.pens{sortfileNum},Units(unitNum).pen)
