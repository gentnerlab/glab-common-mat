...
if iscompressedfile == 1
    if length(trialendinds) ~= length(trialstartinds)
        disp('we have an issue #4: number of trial starts does not equal number of trial ends. I dont know what to do, build better error fixing!');
        return
    end
end
...
for trialnum = 1:size(stimstartinds,1)
    ...
    try
        ...
        keyboard    %hmm, did we try to export a limited time range that didn't include a subfile textmark?
    end
